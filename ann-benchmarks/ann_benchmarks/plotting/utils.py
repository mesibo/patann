...
def generate_n_colors(n):
    vs = np.linspace(0.1, 0.6, 6)
    colors = [(0.6, 0.4, 0.5, 1.0)]

    def euclidean(a, b):
        return sum((x - y) ** 2 for x, y in zip(a, b))

    def color_magnitude(color):
        # Calculate the magnitude of the color vector (ignoring alpha) so that plots are darker
        return np.sqrt(sum(c ** 2 for c in color))

    while len(colors) < n:
        new_color = max(itertools.product(vs, vs, vs), key=lambda a: min(euclidean(a, b) for b in colors))
        new_color_with_alpha = new_color + (1.0,)
        colors.append(new_color_with_alpha)
