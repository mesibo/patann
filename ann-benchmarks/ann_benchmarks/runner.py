...
def run_individual_query(algo: BaseANN, X_train: numpy.array, X_test: numpy.array, distance: str, count: int, 
    ...
    has_batch = hasattr(algo, "batch_query")

    prepared_queries = (has_batch and hasattr(algo, "prepare_batch_query")) or (
        (not has_batch) and hasattr(algo, "prepare_query")
    )
        ...
        def batch_query(X: numpy.array) -> List[Tuple[float, List[Tuple[int, float]]]]:
            ...
            for res in results:
                if len(res) != len(set(res)):
                    print("failed index %d", index)
                    print(res)
                    algo.print_batch_results()
...
def run_docker(
    ...
    container = client.containers.run(
        ...
        network_mode="host",
        #cpuset_cpus=cpu_limit, removed CPU restrictions
        mem_limit=mem_limit,
